...
-- countGates _ (con, pro) func = do
--   let pn = [1..finiteBitSize pro]
--       cn = [1..finiteBitSize con]
--       pcounts = map (const $ Input [] (return $ Counter 0 0 0 0)) pn
--       qcounts = map (const $ Input [] (return $ Counter 0 0 0 0)) cn
--       results = func pcounts qcounts
--       extract Input {keyState = a} = a
